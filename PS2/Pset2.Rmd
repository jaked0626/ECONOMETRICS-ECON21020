...
author: 
  - "Jake Underland"
  - "Collaborated with Jack Surgeoner, Desmond Hui"
date: "`r Sys.Date()`"
output:
  pdf_document: 
...
\[\plim \frac{n-1}{n}\bar{Y} = \plim \frac{n-1}{n} \plim \bar{Y} = 1 \cdot \mu = \mu\]
\[\plim \frac{\bar{Y}}{2} = \frac{1}{2} \plim \bar{Y} = \frac{1}{2} \mu\]
...
Let $\{X_i\}$ be an $iid$ sample drawn from a distribution $F_X$ which has mean $E(X)$. Consider the estimator 
\[\hat{\theta}_n = \Sigma ^n _{i=1} a_i X_i\] 
...
Let $\{X_i\}$ be an $iid$ sample drawn from a distribution $F_X$ which has mean $E(X)$ and variance $Var (X)$. Define the following estimator of $Var (X)$:
\[\hat{\sigma}^2_X = \frac{1}{n} \Sigma^n_{i=1}(X_i-\bar{X}_n)^2 \]
Show that $\hat{\sigma}^2_X$ is consistent.  
...
\end{aligned}\]
Let $g(a, b) = a - b^2$. Then, $g$ is continuous at $(E[X^2], E[X])$. Thus, from the Continuous Mapping Theorem, we have that 
\[\begin{aligned} 
...
# Question 5  
Suppose you observe an $iid$ sample of $n$ observations of the $(2 × 1)$ random vectors
$\{(Yi, Xi)\}^n_{i=1}$ (e.g. an individual’s height and age), drawn from the bivariate distribution $F_{YX}$. Suppose that $Var(X) = \sigma^2_X < \infty$ and $Var(Y) = \sigma^2_Y < \infty$. Consider estimating $E(X)E(Y)$ using the estimator $\bar{X}_n\cdot \bar{Y}_n$.  
